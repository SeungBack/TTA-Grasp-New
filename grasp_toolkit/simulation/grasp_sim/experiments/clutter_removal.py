...
from grasp_sim import io#, vis
from grasp_sim.grasp import *
from grasp_sim.simulation import ClutterRemovalSim
from grasp_sim.utils.transform import Rotation, Transform
from grasp_sim.utils.implicit import get_mesh_pose_list_from_world, get_scene_from_mesh_pose_list
...
def run(
    ...
    for _ in tqdm.tqdm(range(num_rounds), disable=silence):
        ...
        while sim.num_objects > 0 and consecutive_failures < MAX_CONSECUTIVE_FAILURES:
            ...
            # scan the scene
            tsdf, pc, rgb_img, depth_img, timings["integration"] = sim.acquire_tsdf(n=n, N=N, resolution=40)
            state = argparse.Namespace(tsdf=tsdf, pc=pc, rgb=rgb_img, depth=depth_img)
            # if resolution != 40:
        ...
        print('success rate: %.4f, declutter rate: %.4f' % (success / cnt, success / total_objs))
