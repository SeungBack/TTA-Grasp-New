...
class ClutterRemovalSim(object):
    def __init__(self, scene, object_set, gui=True, seed=None, add_noise=False, sideview=False, save_dir=None, save_freq=8):
        ...
        intrinsic = CameraIntrinsic(1920, 1080, 1415, 1424, 969, 548)
    ...
    def acquire_tsdf(self, n, N=None, resolution=40):
        ...
        high_res_tsdf = TSDFVolume(self.size, 200)
        high_res_tsdf_obj = TSDFVolume(self.size, 200)
        ...
        # # Parameters for the XY plane
        # plane_size = 0.5  # Extent of the plane from the origin
        # resolution = 100  # Number of points along one side
        # # Generate points
        # x = np.linspace(-plane_size, plane_size, resolution)
        # y = np.linspace(-plane_size, plane_size, resolution)
        # x, y = np.meshgrid(x, y)
        # z = np.ones_like(x) * np.min(np.asarray(pc.points)[:, 2])
        # # Flatten and create a point cloud
        # xy_plane_points = np.vstack((x.flatten(), y.flatten(), z.flatten())).T
        # xy_plane_cloud = o3d.geometry.PointCloud()
        # xy_plane_cloud.points = o3d.utility.Vector3dVector(xy_plane_points)
        # # add xy plane to point cloud
        # pc += xy_plane_cloud


        return tsdf, pc, rgb_img, depth_img, timing
...
class Gripper(object):
    ...
    def __init__(self, world):
        ...
        self.T_body_tcp = Transform(Rotation.identity(), [0.0, 0.0, -0.018]) # shsh ## -0.012 for blocks default -0.018
