        ...
        if 'mixed' in split or 'mini' in split:
            ...
            for x in tqdm(self.sceneIds, desc = 'Loading data path and collision labels...'):
                for img_num in range(256):
                    ...
                    self.scenename.append(x.strip())
                    self.frameid.append(img_num)
                    self.scene_id_img_id_pairs.append((int(x.split('_')[-1]), int(img_num)))
                if self.load_label:
                    collision_labels = np.load(os.path.join(root, 'collision_label', x.strip(),  'collision_labels.npz'))
                    self.collision_labels[x.strip()] = {}
                    for i in range(len(collision_labels)):
                        self.collision_labels[x.strip()][i] = collision_labels['arr_{}'.format(i)]
        print('Total data size: ', len(self.colorpath))
...
def load_grasp_labels(root):
    ...
    for i, obj_name in enumerate(tqdm(obj_names, desc='Loading grasping labels...')):
        # test_seen
        if int(obj_name) not in [0, 2, 5, 7, 8, 9, 11, 14, 17, 18, 20, 21, 22, 26, 27, 29, 30, 38, 41, 48, 51, 52, 58, 60, 61, 62, 63, 66]:
            continue
        if i == 18: continue
