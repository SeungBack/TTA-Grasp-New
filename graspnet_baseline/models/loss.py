...
def compute_tta_loss(end_points):
    # objectness_loss, end_points = compute_objectness_tta_loss(end_points)
    view_loss, end_points = compute_view_tta_loss(end_points)
    grasp_loss, end_points = compute_grasp_tta_loss(end_points)
    # loss = objectness_loss + view_loss + 0.2 * grasp_loss
    loss = view_loss + 0.2 * grasp_loss
    if 'loss/contrastive_loss' in end_points:
        loss += end_points['loss/contrastive_loss']
    end_points['loss/view_loss'] = view_loss
    end_points['loss/grasp_loss'] = grasp_loss
    end_points['loss/overall_loss'] = loss
    return loss, end_points

def compute_view_tta_loss(end_points):
    criterion = nn.MSELoss(reduction='none')
    view_score = end_points['view_score']
    view_label = end_points['batch_grasp_view_label']
    V = view_label.size(2)
    loss = criterion(view_score, view_label.to(view_score.device))
    loss = loss.mean()
    end_points['loss/stage1_view_loss'] = loss
    return loss, end_points


def compute_grasp_tta_loss(end_points, use_template_in_training=True):
    # objectness_mask = torch.gather(objectness_label, 1, fp2_inds).bool() # (B, Ns)
    ...
    graspable_mask = (target_labels > THRESH_BAD)
    loss_mask = graspable_mask.float()
...
def info_nce_loss(positive_features, negative_features, n_pos_samples=64, n_neg_samples=256, temperature=0.1):
    """
    Implements InfoNCE loss with random sampling to reduce GPU memory requirements
    
    Args:
        positive_features: Tensor of shape [n_pos, 64]
        negative_features: Tensor of shape [n_neg, 64]
        n_pos_samples: Number of positive features to sample
        n_neg_samples: Number of negative features to sample
    ...
    """
    # Normalize features
    positive_features = F.normalize(positive_features, dim=1, p=2)
    negative_features = F.normalize(negative_features, dim=1, p=2)
    
    # Get total number of positive and negative features
    total_pos = positive_features.shape[0]
    total_neg = negative_features.shape[0]
    
    # Ensure we don't sample more than available
    n_pos_samples = min(n_pos_samples, total_pos)
    n_neg_samples = min(n_neg_samples, total_neg)
    
    # Randomly sample positive and negative features
    pos_indices = torch.randperm(total_pos)[:n_pos_samples]
    neg_indices = torch.randperm(total_neg)[:n_neg_samples]
    
    sampled_pos = positive_features[pos_indices]
    sampled_neg = negative_features[neg_indices]
    
    # Compute similarity matrices
    pos_pos_sim = torch.matmul(sampled_pos, sampled_pos.T) / temperature  # [n_pos_samples, n_pos_samples]
    pos_neg_sim = torch.matmul(sampled_pos, sampled_neg.T) / temperature  # [n_pos_samples, n_neg_samples]
    neg_neg_sim = torch.matmul(sampled_neg, sampled_neg.T) / temperature  # [n_neg_samples, n_neg_samples]
    neg_pos_sim = torch.matmul(sampled_neg, sampled_pos.T) / temperature  # [n_neg_samples, n_pos_samples]
    
    # Compute InfoNCE loss
    pos_loss = 0
    for i in range(n_pos_samples):
        # Get similarities with other positive samples (excluding self)
        pos_sim = pos_pos_sim[i, torch.arange(n_pos_samples) != i]
        # Get similarities with negative samples
        neg_sim = pos_neg_sim[i]
        # Combine positive and negative similarities
        logits = torch.cat([pos_sim, neg_sim])
        # Create labels: 1 for positive examples, 0 for negative examples
        labels = torch.cat([torch.ones(n_pos_samples-1), torch.zeros(n_neg_samples)]).to(positive_features.device)
        # Compute cross-entropy loss for this anchor
        anchor_loss = F.cross_entropy(logits.unsqueeze(0), labels.unsqueeze(0))
        pos_loss += anchor_loss
    pos_loss /= n_pos_samples
        
    neg_loss = 0
    for i in range(n_neg_samples):
        # Get similarities with other negative samples (excluding self)
        neg_sim = neg_neg_sim[i, torch.arange(n_neg_samples) != i]
        pos_sim = neg_pos_sim[i]
        # Combine positive and negative similarities
        logits = torch.cat([neg_sim, pos_sim])
        labels = torch.cat([torch.zeros(n_neg_samples-1), torch.ones(n_pos_samples)]).to(negative_features.device)
        # Compute cross-entropy loss for this anchor
        anchor_loss = F.cross_entropy(logits.unsqueeze(0), labels.unsqueeze(0))
        neg_loss += anchor_loss
    neg_loss /= n_neg_samples
    loss = pos_loss + neg_loss
    
    # Average the loss over all anchors
    return loss 
