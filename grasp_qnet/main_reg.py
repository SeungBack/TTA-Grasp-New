...
from dataset.data_loader import GraspEvalDataset, create_balanced_dataloader, create_balanced_grasp_quality_dataloader
...
def train(cfgs):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=cfgs.max_epoch * len(train_dataloader), eta_min=cfgs.min_learning_rate)
    ...
    for name, param in net.named_parameters():
        if param.requires_grad:
            log_string(f'Parameter {name} has {param.numel()/1e6:.3f}M parameters', log_dir)
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        ...
        for batch_idx, (obj_cloud, gripper_cloud, gt_score) in enumerate(tqdm(train_dataloader)):
            ...
            pred = net(obj_cloud.to('cuda'), gripper_cloud.to('cuda'))
            pred = pred.squeeze(-1)
            ...
            if (global_iter + 1) % 2000 == 0 or batch_idx + 1 == total_train_batches:
                ...
                for test_loader, split_name in zip(test_dataloaders, test_splits):
                    evaluate(cfgs, net, test_loader, train_writer, split_name, global_iter)
...
def evaluate(cfgs, net, dataloader, writer, split, iter_num):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--acronym_root', type=str,
                        default= '/home/seung/Datasets/ACRONYM/grasp_qnet',
