...
class Backbone(nn.Module):
    def __init__(self,):
        ...
        npoints, nblocks, nneighbor, d_points = 1024, 4, 16, 3
        ...
        self.transformer1 = TransformerBlock(32, 256, nneighbor)
        self.transition_downs = nn.ModuleList()
        self.transformers = nn.ModuleList()
        for i in range(nblocks):
            channel = 32 * 2 ** (i + 1)
            self.transition_downs.append(TransitionDown(npoints // 4 ** (i + 1), nneighbor, [channel // 2 + 3, channel, channel]))
            self.transformers.append(TransformerBlock(channel, 256, nneighbor))
...
class PointTransformerGraspQNet(nn.Module): # V3
    def __init__(self, ):
        super().__init__()
    ...
    def forward(self, obj_cloud, gripper_cloud):
        ...
        points, _ = self.backbone(obj_cloud)
        obj_feature = torch.cat([points.max(dim=1)[0], points.mean(dim=1)], dim=1)
