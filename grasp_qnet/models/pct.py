...
class PCTGraspQNet(nn.Module):
    ...
    def forward(self, obj_cloud, gripper_cloud):
        ...
        batch_size, _, _ = obj_cloud.size()
        x = self.relu(self.bn1(self.conv1(obj_cloud))) # B, D, N
        x = self.relu(self.bn2(self.conv2(x))) # B, D, N
        x = x.permute(0, 2, 1)
        ...
        combiened_feature = torch.cat([gripper_feature, x], dim=1)
