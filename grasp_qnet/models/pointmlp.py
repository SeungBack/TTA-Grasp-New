# Codes modified from https://github.com/ma-xu/pointMLP-pytorch/blob/main/classification_ModelNet40/models/pointmlp.py
...
class PointMLPGraspQNet(nn.Module):

    def __init__(self):

        super(PointMLPGraspQNet, self).__init__()
        # PointMLP-Elite
        self.obj_backbone = Model(points=1024, embed_dim=32, groups=1, res_expansion=0.25,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 1], pre_blocks=[1, 1, 2, 1], pos_blocks=[1, 1, 2, 1],
                   k_neighbors=[24,24,24,24], reducers=[2, 2, 2, 2])
        # PointMLP
        # self.backbone = Model(points=1024, embed_dim=64, groups=1, res_expansion=1.0,
        #            activation="relu", bias=False, use_xyz=False, normalize="anchor",
        #            dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
        #            k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2])
    ...
    def forward(self, obj_cloud, gripper_cloud):
        ...
        combiened_feature = torch.cat([gripper_feature, obj_feature], dim=1)
        pred_score_cls = self.score_head(combiened_feature)
        return torch.sigmoid(pred_score_cls)