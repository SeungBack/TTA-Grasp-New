...
class PTv3GraspQNet(nn.Module):
    def __init__(self, dropout=0.5, order="z", **kwargs):
        super().__init__()
        ...
        self.score_head = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.2),
            nn.Dropout(dropout),
            nn.Linear(256, 1),  # Changed to output num_classes
            nn.Sigmoid()  # Use Sigmoid for score output
        )
    
    def forward(self, obj_cloud, gripper_cloud):
        
        # concat object and gripper point clouds and append a binary feature
        batch_size = obj_cloud.shape[0]
        obj_feat = torch.zeros((batch_size, obj_cloud.shape[1], 1), device=obj_cloud.device)
        gripper_feat = torch.ones((batch_size, gripper_cloud.shape[1], 1), device=gripper_cloud.device)
        obj_input = torch.cat((obj_cloud, obj_feat), dim=-1)
        pcd_input = torch.cat((obj_input, torch.cat((gripper_cloud, gripper_feat), dim=-1)), dim=1)
        pcd_input = pcd_input.reshape(-1, pcd_input.shape[-1])
        num_points = pcd_input.shape[0] // batch_size
        data_dict = {
            "feat": pcd_input,
            "coord": pcd_input[:, :3],
            "grid_size": 0.005,
            "offset": torch.arange(num_points, (batch_size) * num_points + 1,
                                   num_points, device=pcd_input.device)
            }
        out = self.backbone(data_dict) # [B, C]
