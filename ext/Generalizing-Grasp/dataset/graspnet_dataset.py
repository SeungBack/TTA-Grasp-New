...
class GraspNetDataset(Dataset):
    def __init__(self, root, camera='kinect', split='train', num_points=20000,
                 ...
                 augment=False, load_label=True, return_raw_cloud=False, use_ratio=1.0):
        self.root = root
        self.split = split
        ...
        if use_ratio < 1.0:
            num_scenes = int(len(self.sceneIds) * use_ratio)
            self.sceneIds = self.sceneIds[:num_scenes]
            print(f"Using {num_scenes} scenes from {len(self.sceneIds)} scenes for the {split} split")
        ...
        self.grasp_labels = {}
        self.img_id_per_scene = defaultdict(list)

        if 'mixed' in split or 'mini' in split:
            for x, img_num in tqdm(self.scene_id_img_id_pairs, desc = 'Loading data path and collision labels...'):
                x = 'scene_{}'.format(str(x).zfill(4))
                ...
                for x, img_num in self.scene_id_img_id_pairs:
                    x = 'scene_{}'.format(str(x).zfill(4))
                    self.img_id_per_scene[x].append(img_num)
                # delete the empty scene
                self.img_id_per_scene = {k: v for k, v in self.img_id_per_scene.items() if v}

                if self.load_label:
                    self.grasp_labels[x.strip()] = os.path.join(self.root, 'economic_grasp_label_300views', x + '_labels.npz')

        else:
            self.scene_id_img_id_pairs = []
