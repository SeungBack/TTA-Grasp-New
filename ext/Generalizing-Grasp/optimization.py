...
parser.add_argument('--dataset_root', default='/home/seung/Datasets/GraspNet-1Billion', help='Dataset root')
parser.add_argument('--camera', default='realsense', help='Camera split [realsense/kinect]')
parser.add_argument('--c_checkpoint_path', default='ckpts/contactnet_checkpoint.tar', help='Model checkpoint path [default: None]')
parser.add_argument('--s_checkpoint_path', default='ckpts/scorenet_checkpoint.tar', help='Model checkpoint path [default: None]')
...
TEST_DATASET = GraspNetDataset(cfgs.dataset_root, 
                                      camera = cfgs.camera,
                                      split = 'test',
                                      num_points = 20000, # TODO
                                        voxel_size=0.005, 
                                        remove_outlier=False, 
                                        remove_invisible=True,
                                        augment=False, load_label=False, return_raw_cloud=False, use_ratio=1.0)
    ...
    def opt(self):
        ...
        for batch_idx, batch_data in enumerate(TEST_DATALOADER):
            ...
            for i in range(len(selected_grasp_group)):
                ...
                # select points within 0.1m
                center = selected_grasp_group[i].translation
                dis = np.sqrt(np.sum((point_clouds - center[None]) ** 2, axis=1))
                cropped_points = point_clouds[dis < 0.1]
                # visualize
                o3d_pc = o3d.geometry.PointCloud()
                o3d_pc.points = o3d.utility.Vector3dVector(cropped_points)
                o3d_pc.paint_uniform_color([0, 1, 0])
                full_pc = o3d.geometry.PointCloud()
                full_pc.points = o3d.utility.Vector3dVector(point_clouds)
                o3d.visualization.draw_geometries([o3d_pc, full_pc])
