...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_scene(self, scene_id, dump_folder, img_id_per_scene=None, TOP_K = 50, return_list = False, vis = False, max_width = 0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        for ann_id in range(256):
            if img_id_per_scene is not None:
                scene_key = 'scene_{}'.format(str(scene_id).zfill(4))
                if scene_key not in img_id_per_scene or ann_id not in img_id_per_scene[scene_key]:
                    continue
            if not os.path.exists(os.path.join(dump_folder,get_scene_name(scene_id), self.camera, '%04d.npy' % (ann_id,))):
                print('No grasp accuracy file for scene:{} ann:{}'.format(scene_id, ann_id))
            ...
            np.save(os.path.join(dump_folder, 'scene_{:04d}'.format(scene_id), self.camera, '{:04d}_acc.npy'.format(ann_id)), grasp_accuracy)
            scene_accuracy.append(grasp_accuracy)
        if not return_list:
    ...
    def parallel_eval_scenes(self, scene_ids, dump_folder, img_id_per_scene=None, proc = 2):
        '''
        **Input:**
        ...
        - img_id_per_scene: dict of image ids per scene.
        ...
        **Output:**
        ...
        for scene_id in scene_ids:
            res_list.append(p.apply_async(self.eval_scene, (scene_id, dump_folder, img_id_per_scene)))
        p.close()
        p.join()
        scene_acc_list = []
        for res in res_list:
            scene_acc_list.append(res.get())
        scene_acc_list = [x for x in scene_acc_list if len(x) != 0]
