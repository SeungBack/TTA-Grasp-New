...
class DynamicCylinderQuery(Function):
    @staticmethod
    def forward(ctx, radius, hmin, hmax, nsample, xyz, new_xyz, rot):
        # type: (Any, torch.Tensor, float, float, int, torch.Tensor, torch.Tensor, torch.Tensor) -> torch.Tensor
        return _ext.dynamic_cylinder_query(new_xyz, xyz, rot, radius, hmin, hmax, nsample)
...
dynamic_cylinder_query = DynamicCylinderQuery.apply
