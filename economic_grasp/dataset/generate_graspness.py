...
if __name__ == '__main__':
    ...
    save_path_root = os.path.join(dataset_root, 'graspness_width0.55')

    num_views, num_angles, num_depths = 300, 12, 4
    fric_coef_thresh = 0.8
    point_grasp_num = num_views * num_angles * num_depths
    for scene_id in tqdm(range(0, 100)):
        ...
        for ann_id in range(256):
            ...
            for i, (obj_idx, trans_) in enumerate(zip(obj_list, pose_list)):
                ...
                width = offsets[:, :, :, :, 2]
                valid_grasp_mask = ((fric_coefs <= fric_coef_thresh) & (fric_coefs > 0) & (~collision) & (width <= 0.055))
