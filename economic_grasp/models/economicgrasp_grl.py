...
from .backbone import TDUnet
from .modules_economicgrasp import GraspableNet, ViewNet, Cylinder_Grouping_Global_Interaction, Grasp_Head_Local_Interaction

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(ROOT_DIR + '/utils')
from label_generation import process_grasp_labels, process_grasp_pseudo_label, batch_viewpoint_params_to_matrix
...
class DomainClassifier(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, 1) # Source(0) vs Target(1) 이진 분류
        )
...
def load_economicgrasp(cfg, device):
    ...
    net = EconomicGraspGRL(
        seed_feat_dim=cfg.model.seed_feat_dim,
        is_training=False,
        cfg=cfg,
    )
    net.to(device)
...
class EconomicGraspGRL(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True, voxel_size=0.005, cfg=None):
        super().__init__()
        ...
        self.grl = GradientReverseLayer(weight=-1.0) # weight는 하이퍼파라미터
        # Domain Classifier
        self.domain_classifier = DomainClassifier(input_dim=self.seed_feature_dim)
    ...
    def forward(self, end_points):
        ...
        domain_output = self.domain_classifier(self.grl(seed_features))
        end_points['domain_output'] = domain_output #
        ...
        # Generate the labels
        if self.is_training and 'object_poses_list' in end_points:
        ...
        elif self.is_training and 'object_poses_list' not in end_points:
            return end_points  # no labels for training, just return the end points for domain adaptation 
        elif self.cfg and self.cfg.tta.method in ['tta-grasp', 'cotta']:
            grasp_top_views_rot, end_points = process_grasp_pseudo_label(end_points)
        else:
...
def pred_decode_raw(end_points):
    ...
    for i in range(batch_size):
        ...
        grasp_score_prob = torch.softmax(grasp_score_prob, dim=0)
        score = torch.tensor([0, 0.2, 0.4, 0.6, 0.8, 1]).view(-1, 1).expand(-1, grasp_score_prob.shape[1]).to(grasp_score_prob)
        score = torch.sum(score * grasp_score_prob, dim=0)
        grasp_score = score.view(-1, 1)
    ...
    return grasp_preds, grasp_angles, end_points['view_score'], end_points['graspness_score'], end_points['objectness_score']