...
from grasp_toolkit.utils.cfg_utils import print_cfg
from grasp_toolkit.utils.wandb_utils import init_wandb
from grasp_toolkit.utils.data_utils import set_seed
...
def inference(cfg, graspnet, dataset_name, split, camera):
    ...
    for batch_idx, batch_data in enumerate(tqdm(test_dataloader)):
        ...
        # Forward pass
        tic = time.time()
        grasp_preds, end_points = tta_method.forward_and_adapt(batch_data)
        toc = time.time()
        batch_time = toc - tic
        ...
        for i in range(cfg.tta.batch_size):
            # Set random seed
            ...
            if cfg.model.collision_thresh > 0:
                ...
                mfcdetector = ModelFreeCollisionDetector(cloud, voxel_size=cfg.model.collision_thresh)
...
def test(cfg):
    ...
    if 'infer' in cfg.mode:
        ...
        if 'graspclutter6d' == cfg.tta_dataset.name:
            ...
            model = load_economicgrasp(cfg, device)
            inference(cfg, model, 'graspnet1b', cfg.eval_dataset.split, cfg.eval_dataset.camera)

        if 'graspnet1b' == cfg.tta_dataset.name:
            inference(cfg, model, 'graspnet1b', cfg.tta_dataset.split, cfg.tta_dataset.camera)
...
def sweep_agent(sweep_cfg):
    ...
    cfg.tta.loss_type = wandb.config.loss_type
    cfg.tta.ema_ratio = wandb.config.ema_ratio
    cfg.tta.rst_ratio = wandb.config.rst_ratio
    cfg.tta.lr = wandb.config.lr
    cfg.tta.geval_net.grasp_q_thresh = wandb.config.grasp_q_thresh
